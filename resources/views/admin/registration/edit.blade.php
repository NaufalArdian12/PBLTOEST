...
@section('header', 'Edit Pendaftaran')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="toeic_test_id" id="toeic_test_id"
                            ...
                            @foreach ($toeicTests as $toeicTest)
                                <option value="{{ $toeicTest->id }}" {{ old('toeic_test_id', $registration->toeic_test_id) == $toeicTest->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="date" name="registration_date" id="registration_date"
                            class="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500" value="{{ old('registration_date', $registration->registration_date) }}" required>
                    ...
                    <div>
                        ...
                        <select name="status" id="status"
                            ...
                            <option value="active" {{ old('status', $registration->status) == 'approved' ? 'selected' : '' }}>Approved</option>
                            <option value="inactive" {{ old('status', $registration->status) == 'rejected' ? 'selected' : '' }}>Rejected</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
