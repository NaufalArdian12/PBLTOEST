        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <form action="{{ route('studyprogram.update', $studyProgram->id) }}" method="POST">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="campus_id" id="campus_id"
                            ...
                            @foreach ($campuses as $campus)
                                <option value="{{ $campus->id }}" {{ old('campus_id', $studyProgram->campus_id) == $campus->id ? 'selected' : '' }}>
                                    {{ $campus->campus_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div id="major-section" style="display: {{ $studyProgram->campus_id == 4 ? 'block' : 'none' }};">
                        ...
                        <select name="major_id" id="major_id"
                            ...
                            @foreach ($majors as $major)
                                <option value="{{ $major->id }}" {{ old('major_id', $studyProgram->major_id) == $major->id ? 'selected' : '' }}>
                                    {{ $major->major_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="study_program_name" id="study_program_name"
                            class="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3
                                                                  focus:outline-none focus:ring-blue-500 focus:border-blue-500" value="{{ old('study_program_name', $studyProgram->study_program_name) }}" required>
